...
saveClusterSubset <- function(obj, res, clusters, dir, dataName){
  subset <- SubsetData(obj, subset.name = res, accept.value = clusters)
  save(subset, file = paste0(dir, dataName, ".RData"))
}

createGeneExpressionMetadata <- function(obj, gois, fieldname, threshold=0){
  if (length(gois) > 1){
    newData <- sapply(Matrix::colSums(obj@data[gois,]), function(i){sign(i -threshold)})
  } else {
    newData <- sapply(obj@data[gois,], function(i){sign(i-threshold)})
  }
  return(AddMetaData(obj, newData, col.name = paste0("res.",fieldname)))
}

createGeneExpressionMetadata2 <- function(obj, FUN, fieldname = "newData"){
  newData <- FUN(obj)
  return(AddMetaData(obj, newData, col.name = paste0("res.",fieldname)))  
}

binaryGeneFilt <- function(obj, gois, threshold){
  if (length(gois)> 1){
    return(sapply(Matrix::colSums(obj@data[gois,]), function(i){sign(i-threshold)}))
  } else {
    return(sapply(obj@data[gois,], function(i){sign(i-threshold)}))
  }
}

assignBin <- function(num, thresholds){
  return(min(which(thresholds >= num)))
}

binGeneSubset <- function(obj, gois, thresholds){
  return(sapply(Matrix::colSums(obj@data[gois,,drop=FALSE]), function(i){assignBin(i, thresholds)}))
}

numUniqueSubset <- function(obj, gois){
  return(Matrix::colSums(obj@data[gois,,drop=FALSE] != 0))
}
...
if (!file.exists(paste0(dataPath,"postPCA.RData"))) {
  ...
  eb1S <- ScaleData(eb1S,vars.to.regress=c("Phase"),display.progress=F,check.for.norm=F)
...
maxPCt <- 8 #from elbow plot and spread of PCs.
PCuse <- seq(1,maxPCt) 
...
if (!file.exists(paste0(dataPath,"eb1S.RData"))) {
  ...
  CGS <- deTissue <- deVS <- deMarker <- deNeighb <- deUnfiltered <- clustStats <- cpms <- list()
  resVal <- 0; minNeighbDE <- 100
  for (resVal in seq(0.2,0.4,0.1)){
#  while (minNeighbDE > 0) {
#    if (minNeighbDE <= 30) {
#      resVal <- resVal + 0.1
#    } else {
...
#    }
    ...
    deUnfiltered[[res]] <- sapply(levels(eb1S@ident),function(i)
    ...
    for (i in names(deTissue[[res]])) { 
      deTissue[[res]][[i]]$qVal <- tempQval[[i]] 
      
      cellNames <- rownames(eb1S@meta.data[res][eb1S@meta.data[res] == i, , drop=FALSE])
      deTissue[[res]][[i]]$cpm <- rowMeans(edgeR::cpm(eb1S@data[,cellNames]))[rownames(deTissue[[res]][[i]])]
    }
    ...
    print(paste("# of DE genes between nearest neighbouring clusters:",minNeighbDE))
    
    for (cluster in names(deTissue[[res]])){
      cells <- rownames(eb1S@meta.data[eb1S@meta.data[[res]] == cluster,,drop=FALSE])
      deTissue[[res]][[cluster]]$pct1<- 
        sapply(rownames(deTissue[[res]][[cluster]]), function(i){sum(eb1S@data[i,cells]>0)/ ncol(eb1S@data[,cells]) })
      deTissue[[res]][[cluster]]$pct2 <- 
        sapply(rownames(deTissue[[res]][[cluster]]), function(i){sum(eb1S@data[i,]>0)/ ncol(eb1S@data) })
    }
...
if (!file.exists(paste0(dataPath,eb1S@project.name,"_forViz.RData"))) {
  ...
  save(nge,md,cl,dr_clust,dr_viz,
       CGS,deTissue,deVS,deMarker,deNeighb, deUnfiltered,
